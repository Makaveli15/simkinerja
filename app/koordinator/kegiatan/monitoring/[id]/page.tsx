...
export default function KoordinatorKegiatanDetailPage() {
  ...
  const fetchKegiatan = async () => {
    try {
      const res = await fetch(`/api/koordinator/kegiatan/${id}`);
      if (res.ok) {
        ...
        router.push('/koordinator/kegiatan');
      }
    } catch (error) {
  ...
  if (!kegiatan) {
    return (
      <div className="text-center py-12">
        ...
        <Link href="/koordinator/kegiatan" className="text-green-600 hover:underline mt-2 inline-block">
        ...
        <div className="space-y-6">
          ...
          {(kegiatan.status_pengajuan === 'diajukan' || kegiatan.status_pengajuan === 'review_koordinator') && (
            <Link
              ...
              className="block w-full text-center bg-green-600 text-white px-6 py-3 rounded-xl font-medium hover:bg-green-700 transition-colors"
            >
              Review & Approval
            </Link>
          )}
