...
import { LuSearch, LuClipboard, LuEye, LuActivity, LuTriangleAlert, LuCircleCheck } from 'react-icons/lu';
...
export default function MonitoringKegiatanPage() {
  ...
  const getVerifikasiBadge = (kg: Kegiatan) => {
    ...
    switch (kg.status_verifikasi) {
      case 'valid':
        return <span className="px-2 py-1 text-xs font-medium rounded-full bg-green-100 text-green-700 inline-flex items-center gap-1"><LuCircleCheck className="w-3 h-3" /> Valid</span>;
      case 'revisi':
        return <span className="px-2 py-1 text-xs font-medium rounded-full bg-amber-100 text-amber-700 inline-flex items-center gap-1"><LuTriangleAlert className="w-3 h-3" /> Revisi</span>;
      default:
