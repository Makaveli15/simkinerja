...
export default function KesubagProfilePage() {
  ...
  const fetchProfile = async () => {
    try {
      const res = await fetch('/api/kesubag/profile');
      if (res.ok) {
  ...
  const handleChangePassword = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/kesubag/change-password', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1">
          <div className="bg-white rounded-2xl shadow-lg shadow-gray-200/50 overflow-hidden">
            <div className="h-24 bg-gradient-to-r from-blue-400 via-blue-500 to-indigo-600"></div>
            <div className="px-6 pb-6 -mt-12">
              <div className="w-24 h-24 rounded-2xl bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center text-white shadow-lg overflow-hidden border-4 border-white">
                {formData.foto ? (
                  <img src={formData.foto} alt="" className="w-full h-full object-cover" />
                ) : (
                  <span className="text-3xl font-bold">{user?.username?.charAt(0)?.toUpperCase() || 'K'}</span>
                )}
              </div>
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                <span className="px-3 py-1 bg-gradient-to-r from-blue-500 to-indigo-600 text-white text-xs font-medium rounded-full capitalize">
                  {user?.role}
                </span>
        ...
        <div className="lg:col-span-2">
          <div className="bg-white rounded-2xl shadow-lg shadow-gray-200/50 overflow-hidden">
            ...
            <div className="p-6">
              {/* Message for Profile */}
              {activeTab === 'profile' && message.text && (
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-4">
                      <div className="w-16 h-16 rounded-xl bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center text-white overflow-hidden">
                        {formData.foto ? (
                          <img src={formData.foto} alt="" className="w-full h-full object-cover" />
                        ) : (
                          <span className="text-xl font-bold">{user?.username?.charAt(0)?.toUpperCase() || 'K'}</span>
                        )}
                      </div>
                      <div>
                  ...
                  <div className="flex justify-end">
                    <button
                      type="submit"
                      disabled={isSaving}
                      className="px-6 py-3 bg-gradient-to-r from-blue-500 to-indigo-600 text-white font-medium rounded-xl hover:shadow-lg hover:shadow-blue-500/30 transition-all disabled:opacity-50"
                    >
                      {isSaving ? (
