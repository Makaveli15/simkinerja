...
export default function KesubagLayout({
  children,
}: {
  ...
  const fetchNotifications = useCallback(async () => {
    try {
      const res = await fetch('/api/kesubag/notifications');
      if (res.ok) {
        const data = await res.json();
  ...
  const getNotificationLink = (notif: Notification): string => {
    ...
    if (refType === 'dokumen' && id) {
      return `/kesubag/validasi-dokumen`;
    }
    if (refType === 'kegiatan' && id) {
      return `/kesubag/kegiatan/${id}`;
    }
    ...
    switch (notif.type) {
      ...
      case 'deadline':
      case 'kendala':
        return id ? `/kesubag/kegiatan/${id}` : '/kesubag/kegiatan';
      case 'laporan':
        return '/kesubag/laporan';
      default:
        return '/kesubag/dashboard';
    }
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-blue-50" suppressHydrationWarning>
      ...
      {showFirstLoginModal && (
        <FirstLoginModal 
          ...
          apiEndpoint="/api/kesubag/change-password"
        />
      )}
        ...
        <header className="sticky top-0 z-30 bg-white/80 backdrop-blur-xl border-b border-gray-200/50 shadow-sm">
          <div className="flex items-center justify-between h-16 px-4 lg:px-8">
            ...
            <div className="hidden lg:block">
              <h1 className="text-lg font-semibold text-gray-900">Portal Kesubag</h1>
            </div>
