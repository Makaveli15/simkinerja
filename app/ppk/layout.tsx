...
export default function PPKLayout({
  children,
}: {
  ...
  const fetchNotifications = useCallback(async () => {
    try {
      const res = await fetch('/api/ppk/notifications');
      if (res.ok) {
        const data = await res.json();
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-blue-50" suppressHydrationWarning>
      ...
      {showFirstLoginModal && (
        <FirstLoginModal 
          ...
          apiEndpoint="/api/ppk/change-password"
        />
      )}
        ...
        <header className="sticky top-0 z-30 bg-white/80 backdrop-blur-xl border-b border-gray-200/50 shadow-sm">
          <div className="flex items-center justify-between h-16 px-4 lg:px-8">
            ...
            <div className="hidden lg:block">
              <h1 className="text-lg font-semibold text-gray-900">Portal PPK</h1>
              <p className="text-sm text-gray-500">Pejabat Pembuat Komitmen</p>
            </div>
