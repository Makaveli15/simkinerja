...
export default function PPKProfilePage() {
  ...
  const fetchProfile = async () => {
    try {
      const res = await fetch('/api/ppk/profile');
      if (res.ok) {
  ...
  const handleChangePassword = async () => {
    ...
    try {
      const res = await fetch('/api/ppk/change-password', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          ...
          <div className="mt-8">
            {editMode ? (
              <input
                ...
                className="text-2xl font-bold text-gray-900 border-b-2 border-orange-500 focus:outline-none w-full"
              />
            ) : (
              <h2 className="text-2xl font-bold text-gray-900">{profile.nama}</h2>
            )}
            <p className="text-orange-600 font-medium mt-1">Pejabat Pembuat Keputusan (PPK)</p>
          </div>
              ...
              <div className="flex-1">
                ...
                {editMode ? (
                  <input
                    ...
                    className="font-medium text-gray-900 border-b border-gray-300 focus:border-orange-500 focus:outline-none w-full"
                  />
                ) : (
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-6">
        ...
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className="px-4 py-2 text-orange-600 hover:bg-orange-50 rounded-xl transition-colors"
          >
            Ubah Password
          </button>
        </div>
