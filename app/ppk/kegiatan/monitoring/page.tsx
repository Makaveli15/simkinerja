...
export default function PPKKegiatanPage() {
  ...
  const [timList, setTimList] = useState<{ id: number; nama: string }[]>([]);
  ...
  useEffect(() => {
    filterKegiatan();
  }, [kegiatan, searchQuery, statusFilter, timFilter]);

  const fetchKegiatan = async () => {
    try {
      const res = await fetch('/api/ppk/kegiatan?all=true');
      if (res.ok) {
        const data = await res.json();
        setKegiatan(data.kegiatan || []);
        // Extract unique teams
        const teams = [...new Map(data.kegiatan?.map((k: Kegiatan) => [k.tim_nama, { id: k.tim_id, nama: k.tim_nama }]) || []).values()];
        setTimList(teams as { id: number; nama: string }[]);
      }
    } catch (error) {
  ...
  const filterKegiatan = () => {
    ...
    if (timFilter !== 'all') {
      filtered = filtered.filter(k => k.tim_nama === timFilter);
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-gray-600">Pantau semua kegiatan yang telah disetujui</p>
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-4">
        ...
        <select
          ...
          className="px-4 py-2.5 border border-gray-200 rounded-xl focus:ring-2 focus:ring-orange-500 focus:border-transparent"
        >
        ...
        >
          ...
          {timList.map(tim => (
            <option key={tim.id} value={tim.nama}>{tim.nama}</option>
          ))}
        </select>
      </div>
