...
export async function POST(
  ...
  try {
    ...
    const { tanggal_realisasi, jumlah, keterangan } = body;

    if (!tanggal_realisasi || jumlah === undefined) {
    ...
    return NextResponse.json({ 
      message: 'Realisasi anggaran berhasil ditambahkan',
      id: result.insertId 
    }, { status: 201 });
  } catch (error) {
    console.error('Error adding realisasi anggaran:', error);
