...
// POST - Bulk validate all pending documents (pimpinan)
export async function POST(req: NextRequest) {
  try {
    ...
    const payload = JSON.parse(authCookie.value);
    if (!payload || payload.role !== 'pimpinan') {
      return NextResponse.json({ error: 'Forbidden - Only pimpinan can bulk validate' }, { status: 403 });
    }
    ...
    if (!action || !['valid', 'draft_feedback'].includes(action)) {
      return NextResponse.json({ error: 'Action must be "valid" or "draft_feedback"' }, { status: 400 });
    }

    // Get all pending documents for this kegiatan that are ready for pimpinan
    const [pendingDocs] = await pool.query<RowDataPacket[]>(`
      ...
      WHERE d.kegiatan_id = ?
      AND (
        (d.tipe_dokumen = 'final' AND d.minta_validasi = 1 AND d.validasi_kesubag = 'valid' AND (d.validasi_pimpinan = 'pending' OR d.validasi_pimpinan IS NULL))
        OR
    ...
    for (const doc of pendingDocs) {
      const isFinal = doc.tipe_dokumen === 'final' && doc.minta_validasi === 1 && doc.validasi_kesubag === 'valid';
      const isDraft = doc.tipe_dokumen === 'draft' && doc.draft_status_kesubag === 'reviewed';

      if (isFinal && action === 'valid') {
        // Validate and sahkan final document
        await pool.query<ResultSetHeader>(`
          UPDATE dokumen_output 
          SET 
            validasi_pimpinan = 'valid',
            validasi_feedback_pimpinan = ?,
            validasi_by_pimpinan = ?,
            validasi_at_pimpinan = NOW(),
            status_final = 'disahkan',
            tanggal_disahkan = NOW()
          WHERE id = ?
        `, [catatan || 'Divalidasi dan disahkan melalui validasi massal', payload.id, doc.id]);
        
        processedCount++;
        uniqueUploaders.add(doc.uploaded_by);
      } else if (isDraft && action === 'draft_feedback') {
        // Give feedback on draft document
        await pool.query<ResultSetHeader>(`
          UPDATE dokumen_output 
          SET 
            draft_feedback_pimpinan = ?,
    ...
    // Update kegiatan status if final documents were validated
    if (action === 'valid' && processedCount > 0) {
      await pool.query(
        'UPDATE kegiatan SET status_verifikasi = ? WHERE id = ?',
        ['valid', kegiatan_id]
      );
    ...
    for (const uploaderId of uniqueUploaders) {
      await createNotification({
        userId: uploaderId,
        title: action === 'valid' ? '🏆 Dokumen Disahkan!' : '💬 Feedback Draft dari Pimpinan',
        message: action === 'valid'
          ? `Beberapa dokumen untuk kegiatan "${kegiatanNama}" telah divalidasi dan disahkan oleh Pimpinan melalui validasi massal.`
          : `Pimpinan memberikan feedback untuk beberapa draft pada kegiatan "${kegiatanNama}" melalui review massal.`,
        type: 'validasi',
    ...
    return NextResponse.json({
      message: `${processedCount} dokumen berhasil ${action === 'valid' ? 'divalidasi dan disahkan' : 'direview'}`,
      success: true,
