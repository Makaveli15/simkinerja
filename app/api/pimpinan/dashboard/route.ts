...
// GET - Dashboard data for pimpinan (read-only)
export async function GET(req: NextRequest) {
  try {
    ...
    // Skor Kinerja Distribution (for chart)
    const skorDistribution = {
      '0-20': kegiatanWithKinerja.filter(k => k.skor_kinerja >= 0 && k.skor_kinerja < 20).length,
      '20-40': kegiatanWithKinerja.filter(k => k.skor_kinerja >= 20 && k.skor_kinerja < 40).length,
      '40-60': kegiatanWithKinerja.filter(k => k.skor_kinerja >= 40 && k.skor_kinerja < 60).length,
      '60-80': kegiatanWithKinerja.filter(k => k.skor_kinerja >= 60 && k.skor_kinerja < 80).length,
      '80-100': kegiatanWithKinerja.filter(k => k.skor_kinerja >= 80 && k.skor_kinerja <= 100).length
    };
    ...
    // Get pending kendala (unresolved issues)
    ...
    const pendingVerifikasi = kegiatanWithKinerja.filter(k => k.status_verifikasi === 'belum_verifikasi').length;
